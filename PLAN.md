...
- [x] 目玉カスタム = **エージェントシステム再編（宣言的エージェント定義）**（2026-08-07 確定・デモ MVP → 本格実装は後）

---

## 構想: ワークスペース単位の宣言的エージェント定義（2026-08-07・**目玉カスタムに確定**・デモ MVP はハッカソンに、本格実装は後）

### ハッカソンデモのスコープ（MVP）
- ワークスペースの agent definition（JSONC）を OverseerDO に保存 → agent.ts が buildAgent 時に読み込み
- `tools.enabled` でツールフィルタ（spawnerConfig 分岐 @2834 と同じ機構）+ `prompts` 断片を動的スロット注入
- UI は最小限（Agent タブ or シード定義 + チャット実演）。skills・model 制御・本格タブは後
- ライブ実演例: 「このワークスペースでは webFetch を無効化」→ チャットでツールが変わる
